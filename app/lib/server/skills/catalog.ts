...
export function resolveSkillCatalogRoots(options: {
  workspaceRoot?: string;
  codexHome?: string;
} = {}): SkillCatalogRoot[] {
  const workspaceRoot = path.resolve(options.workspaceRoot ?? process.cwd(), AGENT_SKILLS_DIRECTORY_NAME);
  const codexHome = resolveCodexHomeDirectory(options.codexHome);
  const codexHomeRoot = path.resolve(codexHome, AGENT_SKILLS_DIRECTORY_NAME);

  const roots: SkillCatalogRoot[] = [
    { path: workspaceRoot, source: "workspace" },
    { path: codexHomeRoot, source: "codex_home" },
  ];
...
export async function discoverSkillCatalog(options: {
  workspaceRoot?: string;
  codexHome?: string;
...
export function resolveCodexHomeDirectory(codexHome?: string): string {
  ...
  const envConfigured = typeof process.env.CODEX_HOME === "string" ? process.env.CODEX_HOME.trim() : "";
  if (envConfigured) {
